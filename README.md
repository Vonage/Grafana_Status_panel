...
It is very similar to the Single Stat panel, but it can hold multiple values pulled from different data sources.
Each value can be used to customize the panel in different ways: 
* Mark the severity of the server
* Mark if the server is disabled
* Show extra data in the panel about the server  
...
This plugin will make it easier to do. You just add all the measurements you want to track, and choose how you want their values to be treated:
1. Server severity marker - Set the threshold for each and you get an overview that will report to you if there is anything wrong with any measurements. That means that if all the measurements are in the OK zone, the panel will be green. If evan one of the measurements is in the WARNING zone, it will be yellow, and red if any of them is CRITICAL.
2. Server disable marker - Set the exact value that represent if the server is disabled, the panel will be grey.
3. Annotation - show extra information about the server in the top left side of the panel.

Notice! The disable markers are prioritized over the severity ones. 
...
3. Go the the Options tab, and choose the how to treat each metric. 
	1. For severity disply, select the `Threshold` option type under `Status Type`. Enter the `Warning` and `Critical` thresholds for each of your queries.
		* If you want the result to always be displayed with it's value (regardless to the thresholds), check the box titled `Show Value`
 		* The plugin automatically detects if higher values are good, or lower values are good by checking which threshold is higher/lower. i.e. if in your measurement higher values are better, put a lower value in the "critical" threshold than the "warning" threshold. 
